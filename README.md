...
└── webapp.gs        # 웹앱 라우팅(doGet), 페이지 전환, 사용자 권한 및 데이터 CRUD API
```
...
| 열 이름 | 유형 | 설명 |
| --- | --- | --- |
| ID | String (UUID) | 명함의 고유 식별자 |
| 생성일시 | ISO 8601 String | 등록 또는 최종 업데이트 시간 |
| 회사명 | String | 기업명 |
| 이름 | String | 소유자 이름 |
| 직책 | String | 소유자 직책 |
| 전화번호 | String | 회사 유선 전화번호 (숫자와 - 조합) |
| 휴대폰 | String | 개인 모바일 번호 |
| 이메일 | String | 이메일 주소 |
| 주소 | String | 회사 주소 |
| 명함이미지링크 | String (URL) | 구글 드라이브 내 이미지 뷰어 링크 |
| 처리상태 | String | OK (신규 등록) 또는 UPDATED (중복 업데이트) |
...
승인된 내부 인원 리스트입니다. (`이메일 / 이름 / 역할(admin 또는 user) / 등록일시`)
...
2. 상단 메뉴에서 확장 프로그램 → Apps Script를 클릭하여 코드 편집기를 엽니다.
3. 프로젝트 내 모든 .gs 파일 내용과 .html 파일을 복사하여 Apps Script 프로젝트에 동일하게 추가합니다.
4. initSheet.gs 파일로 이동한 후, initSheet 함수를 실행하여 시스템 필수 데이터베이스 시트들을 자동으로 생성합니다. (최초 실행 시 Google 권한 승인이 필요합니다.)

### 단계 2: 구글 드라이브 폴더 준비 및 세팅

1. 구글 드라이브 루트 폴더에 Namecard_Images 이름으로 새 폴더를 생성합니다.
2. 명함 이미지 업로드 시 앱이 해당 폴더에서 파일을 스캔합니다. (처리 완료된 파일은 Namecard_Done 폴더로 자동 이동됩니다.)

### 단계 3: Gemini API Key 등록

1. Google AI Studio에서 개인 Gemini API Key를 발급받습니다.
2. namecard.gs 파일 상단의 CONFIG 오브젝트 내 GEMINI_API_KEY 값을 발급받은 API 키로 대체합니다.
...
1. Apps Script의 trigger.gs 파일로 이동합니다.
2. createTimeTrigger 함수를 한 번 실행합니다.
3. 구글 트리거 정책에 따라 1분 주기로 백엔드 스크립트가 실행되며 Namecard_Images에 새 파일이 올라왔는지 지속적으로 모니터링합니다.

### 단계 5: 웹 애플리케이션으로 웹앱 배포

1. Apps Script 편집기 우측 상단의 배포 → 새 배포를 클릭합니다.
2. 유형 선택에서 웹 앱을 선택합니다.
3. 다음과 같이 설정 정보를 기입합니다.
   - 설명: CardVault Pro v1.0.0
   - 웹앱을 실행할 사용자: 웹앱에 액세스하는 사용자 (인증된 Google 계정 필수)
   - 액세스 권한이 있는 사용자: 모든 사용자 또는 회사 환경에 맞게 지정
4. 배포 버튼을 누르고 생성된 웹앱 URL을 복사하여 즐겨찾기 또는 홈 화면에 저장해 사용합니다.
